...
public class XxlJobUtils {
    ...
    public XxlJobInfo buildXxlJobInfo(MessageTemplate messageTemplate){
        ...
        if (messageTemplate.getExpectPushTime().equals(String.valueOf(CommonConstant.FALSE))) {
            scheduleConf = DateUtil.format(DateUtil.offsetSecond(new Date(), XxlJobConstant.DELAY_TIME),
                    CommonConstant.CRON_FORMAT);
        }
            XxlJobInfo xxlJobInfo = XxlJobInfo.builder()
                    .jobGroup(queryJobGroupId())
                    .jobDesc(messageTemplate.getName()) // 执行器
    ...
    private Integer queryJobGroupId() {
        ...
        if (Objects.isNull(basicResultVO.getData())){
            ...
            XxlJobGroup xxlJobGroup = XxlJobGroup.builder()
                    .appname(appName).title(jobHandlerName).addressType(CommonConstant.FALSE).build();
            // 若响应状态正确
